...
export async function exportData(): Promise<void> {
  const [exercises, workouts, programs, sessions, rowingPrograms, rowingProgress, rowingSessions, bodyWeight] = await Promise.all([
  ...
  const data = { exercises, workouts, programs, sessions, rowingPrograms, rowingProgress, rowingSessions, bodyWeight, exportedAt: new Date().toISOString() };
...
export async function importData(file: File): Promise<void> {
  ...
  await db.transaction('rw', [db.exercises, db.workouts, db.programs, db.sessions, db.rowingPrograms, db.rowingProgress, db.rowingSessions, db.bodyWeight], async () => {
